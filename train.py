...
def main(opt):
    ...
    train_loader, test_loader = get_loaders("../VOCdevkit/VOC2012/ImageSets/Main/trainval.txt",
                                            "../VOCdevkit/VOC2012/ImageSets/Main/trainval.txt",batch_size)
    nbs = 64
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, (imgs, targets) in pbar:
            ...
            with torch.cuda.amp.autocast():
                output = model(imgs)
                loss, noobjl, objl, classl, coordinatel = loss_fn(output, targets)

            scaler.scale(loss).backward()

            losses.append(loss.item())  # item()什么用？ 将torch类型转化为浮点数
            noobjles.append(noobjl.item())
            objles.append(objl.item())
            classles.append(classl.item())
            coordinateles.append(coordinatel.item())
            ...
            mean_noob = sum(noobjles) / len(losses)
            mean_obj = sum(objles) / len(losses)
            mean_class = sum(classles) / len(losses)
            mean_coord = sum(coordinateles) / len(losses)
            pbar.set_postfix(loss = mean_loss, noobjloss = mean_noob, objloss = mean_obj, classloss = mean_class, corrdinateloss = mean_coord)
            pbar.update(1)
#            lr = [x['lr'] for x in optimizer.param_groups]  # for loggers
        ...
        if epoch > 10 and epoch % 3 ==0:
            ...
            model.eval()
            meanAP = get_mAP("../VOCdevkit/VOC2012/detfile", "../VOCdevkit/VOC2012/Annotations/{}.xml", "../VOCdevkit/VOC2012/ImageSets/Main/trainval.txt", "../VOCdevkit/VOC2012/cache", test_loader, model,img_size)
            if meanAP > best_fitness:
                best_fitness = meanAP
